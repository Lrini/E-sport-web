...
<div class="overflow-hidden bg-white rounded-lg shadow-lg">
    ...
    <div class="p-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                <button id="open-lomba-modal" class="px-4 py-2 font-medium text-white transition-colors duration-200 rounded-md bg-primary hover:bg-primary/90">
                    Tambah Lomba Baru
                </button>
            </div>
            <div class="text-sm text-muted-foreground">
                Total: <span id="total-count">0</span> lomba
            </div>
        </div>
        <div class="overflow-x-auto">
            <table id="lomba-table" class="w-full border-collapse table-auto">
...
#lomba-table_wrapper .dataTables_length,
#lomba-table_wrapper .dataTables_filter,
#lomba-table_wrapper .dataTables_info,
#lomba-table_wrapper .dataTables_paginate {
    margin-bottom: 1rem;
    color: #6b7280;
    font-size: 0.875rem;
}

#lomba-table_wrapper .dataTables_length select,
#lomba-table_wrapper .dataTables_filter input {
...
#lomba-table_wrapper .dataTables_length select:focus,
#lomba-table_wrapper .dataTables_filter input:focus {
    outline: none;
    border-color: #3b82f6;
...
#lomba-table_wrapper .dataTables_paginate .paginate_button {
    padding: 0.5rem 0.75rem;
    margin: 0 0.125rem;
    border: 1px solid #d1d5db;
    border-radius: 0.375rem;
...
#lomba-table_wrapper .dataTables_paginate .paginate_button:hover {
    background: #f3f4f6;
    border-color: #9ca3af;
    color: #111827;
}

#lomba-table_wrapper .dataTables_paginate .paginate_button.current {
    background: #3b82f6;
    border-color: #3b82f6;
    color: #ffffff;
}

#lomba-table_wrapper .dataTables_paginate .paginate_button.disabled {
    background: #f9fafb;
    border-color: #e5e7eb;
    color: #9ca3af;
    cursor: not-allowed;
...
#lomba-table tbody tr {
    transition: background-color 0.15s ease-in-out;
}

#lomba-table tbody tr:hover {
    background-color: #f8fafc;
}

#lomba-table tbody tr:nth-child(even) {
    background-color: #f9fafb;
}

#lomba-table tbody tr:nth-child(odd) {
    background-color: #ffffff;
}

#lomba-table_processing {
    position: absolute;
    top: 50%;
    left: 50%;
...
$(function() {
    ...
    // Modal functionality
    $('#open-lomba-modal').on('click', function() {
        $('#lomba-modal').removeClass('hidden');
    });

    $('#close-lomba-modal, #cancel-lomba').on('click', function() {
        $('#lomba-modal').addClass('hidden');
    });

    // Close modal when clicking outside
    $('#lomba-modal').on('click', function(e) {
        if (e.target === this) {
